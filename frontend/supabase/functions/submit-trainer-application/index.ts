...
const trainerSchema = z.object({
  businessName: z.string().trim().min(1, "Professional name is required").max(100),
...
serve(async (req) => {
  ...
  try {
    ...
    const validationResult = trainerSchema.safeParse(body)
    ...
    // Insert the trainer application
    ...
    try {
      ...
      const emailResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `Nueva solicitud de Entrenador: ${escapeHtml(contactName)}`,
          html: `
            <h2>Nueva solicitud de Entrenador/Profesional</h2>
            <p><strong>Nombre Profesional:</strong> ${escapeHtml(businessName)}</p>
            <p><strong>Nombre Completo:</strong> ${escapeHtml(contactName)}</p>
            <p><strong>Email:</strong> ${escapeHtml(email)}</p>
            <p><strong>Teléfono:</strong> ${escapeHtml(phone)}</p>
            <p><strong>Años de Experiencia:</strong> ${experienceYears}</p>
            ${website ? `<p><strong>Portfolio/LinkedIn:</strong> <a href="${escapeHtml(website)}">${escapeHtml(website)}</a></p>` : ''}
            <p><strong>Experiencia y Especialización:</strong></p>
