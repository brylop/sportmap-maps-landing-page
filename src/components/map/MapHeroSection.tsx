...
import { Search, Filter, MapPin, GraduationCap, Users, Route, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { InteractiveMap } from './InteractiveMap';
import { MapSidebar } from './MapSidebar';
import { MapLocation, SportRoute, mapStats } from '@/data/mapData';
...
export function MapHeroSection({ onScrollToFeatures }: MapHeroSectionProps) {
  ...
  return (
    <section className="relative w-full h-[85vh] min-h-[600px] overflow-hidden">
      ...
      {/* Search & Filters Bar */}
      <div className="absolute top-20 left-1/2 -translate-x-1/2 w-full max-w-3xl px-4 z-20">
        <motion.div 
        ...
        >
          ...
          <div className="relative">
            ...
            <Input
              type="text"
              placeholder="Busca academias, canchas, rutas o entrenadores cerca de ti..."
              value={searchQuery}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6 md:gap-8">
              <div className="text-center">
                <span className="text-2xl md:text-3xl font-bold text-sport-primary">{mapStats.academies}+</span>
                <p className="text-xs text-sport-text-muted">Academias</p>
              </div>
              <div className="w-px h-10 bg-sport-border hidden md:block" />
              <div className="text-center">
                <span className="text-2xl md:text-3xl font-bold text-sport-accent">{mapStats.routes}+</span>
                <p className="text-xs text-sport-text-muted">Rutas</p>
              </div>
              <div className="w-px h-10 bg-sport-border hidden md:block" />
              <div className="text-center">
                <span className="text-2xl md:text-3xl font-bold text-sport-highlight">{mapStats.cities}</span>
